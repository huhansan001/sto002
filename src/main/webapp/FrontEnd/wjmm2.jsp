...
<html>
	...
	<body>
						...
						<div class="telContent">
							<div class="fromBox" id="telBox">
								...
								<p>
                        			...
                        			<a class="chooseBtn s_next" href="#" id="xiayibu">下一步</a>
								</p>
							</div>
						</div>
					</div>
		...
		<script>
		$(function() {
			$("#sendFGPWDPhone").css("display","none");
			$("#iii").click(function(){
				$("#upshouji").text("邮箱：");
				$("#yanzNum").text("验证码：");
				$("#iii").attr('src','img/red.png');
				$("#ppp").attr('src','img/red2.png');
				$("#sendFGPWDPhone").css("display","");
			});
			$("#ppp").click(function() {
				$("#upshouji").text("输入密码：");
				$("#yanzNum").text("确认新密码：");
			...
			$("#xiayibu").click(function(){
				var pass1 = $("#mobile").val();
				var pass2 = $("#code").val();
				if(pass1 == pass2){
					$.ajax({
						type:"post",
						url:"http://localhost:8080/stoMaven/updateUserRealname_pass.action",
						data:"userPass="+pass2+"&phoneNumber="+getCookie("phoneNumber"),
						success:function(m){
							alert(m);
				...
				alert("点了"+getCookie("phoneNumber"));
			})
		</script>
	</body>
</html>